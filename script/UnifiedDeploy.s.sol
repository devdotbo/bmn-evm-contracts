...
contract UnifiedDeploy is Script {
    ...
    function run() external {
        ...
        console.log("========================================");
        console.log("Deploying on", getNetworkType(chainId));
        ...
        if (isChainA(chainId)) {
            // Chain A (Base): Alice has tokenA, Bob needs tokenA as resolver
            console.log(getChainName(chainId), "token distribution:");
        ...
        } else if (isChainB(chainId)) {
            // Chain B (Etherlink): Bob has tokenB for liquidity
            console.log(getChainName(chainId), "token distribution:");
    ...
    function isChainA(uint256 chainId) internal pure returns (bool) {
        return chainId == BASE_SEPOLIA_CHAIN_ID || chainId == BASE_MAINNET_CHAIN_ID;
    }

    function isChainB(uint256 chainId) internal pure returns (bool) {
        return chainId == ETHERLINK_TESTNET_CHAIN_ID || chainId == ETHERLINK_MAINNET_CHAIN_ID;
    }

    function getNetworkType(uint256 chainId) internal pure returns (string memory) {
        if (chainId == BASE_SEPOLIA_CHAIN_ID || chainId == ETHERLINK_TESTNET_CHAIN_ID) {
            return "Testnet";
        }
        if (chainId == BASE_MAINNET_CHAIN_ID || chainId == ETHERLINK_MAINNET_CHAIN_ID) {
            return "Mainnet";
        }
