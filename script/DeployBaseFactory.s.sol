...
contract DeployBaseFactory is Script {
    address constant CREATE3_FACTORY = 0x7B9e9BE124C5A0E239E04fDC93b66ead4e8C669d;
    bytes32 constant FACTORY_SALT = keccak256("BMN-BaseFactory-MAINNET-v1");
    ...
    function run() external {
        ...
        console.log("[DEPLOY BASE FACTORY]");
        ...
        console.log("\nTarget BaseFactory:", factoryAddr);
        ...
        // Deploy BaseEscrowFactory (simpler, no bytecode validation)
        if (factoryAddr.code.length == 0) {
            ...
            console.log("[DEPLOYED] BaseEscrowFactory:", deployed);
            require(deployed == factoryAddr, "Address mismatch");
        } else {
            console.log("BaseEscrowFactory already deployed");
        }
        ...
        console.log("BaseEscrowFactory:", factoryAddr);
        console.log("EscrowSrc:", srcImpl);
        console.log("EscrowDst:", dstImpl);
        console.log("===================================");
        ...
        string memory deploymentData = string(abi.encodePacked(
            "# BMN MAINNET DEPLOYMENT - ", getChainName(), "\n",
            "CHAIN_ID=", vm.toString(block.chainid), "\n",
            "BASE_FACTORY=", vm.toString(factoryAddr), "\n",
