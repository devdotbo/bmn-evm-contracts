...
# Using multiple grep -v to remove various warning types while preserving actual errors
echo "$OUTPUT" | grep -v "Warning: Multi chain deployment is still under development" \
    ...
    | grep -v "contains a transaction to .* which does not contain any code" || echo "$OUTPUT"
