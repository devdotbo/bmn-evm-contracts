...
if check_port 8545 && check_port 8546; then
    ...
    if [ -f "deployments/chainA.json" ] && [ -f "deployments/chainB.json" ]; then
        ...
        if [ ! -z "$TOKEN_A_CHAIN_A" ]; then
            # Token A on Chain A
            ALICE_TKA=$(cast call --block-number latest $TOKEN_A_CHAIN_A "balanceOf(address)(uint256)" $ALICE --rpc-url http://localhost:8545 2>/dev/null | awk '{print $1}')
            BOB_TKA=$(cast call --block-number latest $TOKEN_A_CHAIN_A "balanceOf(address)(uint256)" $BOB --rpc-url http://localhost:8545 2>/dev/null | awk '{print $1}')
        ...
        if [ ! -z "$TOKEN_B_CHAIN_B" ]; then
            # Token B on Chain B
            ALICE_TKB=$(cast call --block-number latest $TOKEN_B_CHAIN_B "balanceOf(address)(uint256)" $ALICE --rpc-url http://localhost:8546 2>/dev/null | awk '{print $1}')
            BOB_TKB=$(cast call --block-number latest $TOKEN_B_CHAIN_B "balanceOf(address)(uint256)" $BOB --rpc-url http://localhost:8546 2>/dev/null | awk '{print $1}')
